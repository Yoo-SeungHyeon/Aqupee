...
# 최소/최대 온도 계산
min_temp = temp_data['월평균 기온'].min()
max_temp = temp_data['월평균 기온'].max()
...
filenames = []
for (year, month), group in temp_data.groupby(['년도', '월']):
    m = folium.Map(location=[36.5, 128], zoom_start=7, tiles='cartodbpositron')

    for index, row in group.iterrows():
        if row['지점 번호'] in coordinates:
            lat, lon = coordinates[row['지점 번호']]
            # 온도에 따른 색상 적용
            norm_temp = (row['월평균 기온'] - min_temp) / (max_temp - min_temp)  # 정규화
            color = mcolors.rgb2hex(cmap(norm_temp))  # 색상 변환
            radius = 10000 + (row['월평균 기온'] * 100)  # 온도가 높을수록 크게 표시
            folium.Circle(
                location=[lat, lon],
                radius=radius,
                color=color,
            ...
            folium.Marker(
                location=[lat, lon],
                icon=folium.DivIcon(html=f'<div style="font-size: 12pt; color: black; text-align: center;">{row["월평균 기온"]:.1f}°C</div>')
            ).add_to(m)
